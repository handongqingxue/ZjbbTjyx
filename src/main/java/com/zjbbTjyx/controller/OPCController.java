...
public class OPCController {
	...
	public Map<String, Object> keepWatchOnTriggerVar() {
		...
		try {
			...
			//助剂六一次添加完成
			String zjlyctjwcTVVarNamePre=Constant.ZHU_JI_LIU_YI_CI_TIAN_JIA_WAN_CHENG;
			List<TriggerVar> zjlyctjwcTVList = (List<TriggerVar>)triggerVarMap.get(zjlyctjwcTVVarNamePre);//获取助剂六一次添加完成变量,不管是否是上升沿
			List<TriggerVar> upZjlyctjwcTVList = getUpDownVarValueListFromList(zjlyctjwcTVList, TriggerVar.UP);//获取上升的助剂六一次添加完成变量
			...
			//助剂六二次添加完成
			String zjlectjwcTVVarNamePre=Constant.ZHU_JI_LIU_ER_CI_TIAN_JIA_WAN_CHENG;
			List<TriggerVar> zjlectjwcTVList = (List<TriggerVar>)triggerVarMap.get(zjlectjwcTVVarNamePre);//获取助剂六二次添加完成变量,不管是否是上升沿
			List<TriggerVar> upZjlectjwcTVList = getUpDownVarValueListFromList(zjlectjwcTVList, TriggerVar.UP);//获取上升的助剂六二次添加完成变量
			...
			List<TriggerVar> upEcjxlhstxTVList = getUpDownVarValueListFromList(ecjxlhstxTVList, TriggerVar.DOWN);//获取下降的二次加小料和水提醒变量
			for (TriggerVar upEcjxlhstxTV : upEcjxlhstxTVList) {
				Integer upFId = upEcjxlhstxTV.getFId();//获取反应釜号
				switch (upFId) {//匹配反应釜号
					case Constant.F1_ID:
						Map<String,Object> paramF1Map=new HashMap<String,Object>();
						paramF1Map.put("tvVarNamePre",ecjxlhstxTVVarNamePre);
						paramF1Map.put("upEcjxlhstxTV",upEcjxlhstxTV);
						paramF1Map.put("preValueFUMap",preValueF1UMap);
						addProVarByParamMap(paramF1Map);
						break;
					case Constant.F2_ID:
						...
						paramF2Map.put("upEcjxlhstxTV",upEcjxlhstxTV);
						paramF2Map.put("preValueFUMap",preValueF2UMap);
						addProVarByParamMap(paramF2Map);
						break;
					case Constant.F3_ID:
						...
						paramF3Map.put("upEcjxlhstxTV",upEcjxlhstxTV);
						paramF3Map.put("preValueFUMap",preValueF3UMap);
						addProVarByParamMap(paramF3Map);
						break;
					case Constant.F4_ID:
						...
						paramF4Map.put("upEcjxlhstxTV",upEcjxlhstxTV);
						paramF4Map.put("preValueFUMap",preValueF4UMap);
						addProVarByParamMap(paramF4Map);
						break;
					case Constant.F5_ID:
						...
						paramF5Map.put("upEcjxlhstxTV",upEcjxlhstxTV);
						paramF5Map.put("preValueFUMap",preValueF5UMap);
						addProVarByParamMap(paramF5Map);
						break;
				}
			...
			//二次加水启动
			String ecjsqdTVVarNamePre=Constant.ER_CI_TOU_FEN;
			List<TriggerVar> ecjsqdTVList = (List<TriggerVar>)triggerVarMap.get(ecjsqdTVVarNamePre);//获取二次加水启动,不管是否是上升沿
			List<TriggerVar> upEcjsqdTVList = getUpDownVarValueListFromList(ecjsqdTVList, TriggerVar.DOWN);//获取下降的二次加水启动变量
...
//						paramF5Map.put("preValueFUMap",preValueF5UMap);
	...
	private void addProVarByParamMap(Map<String,Object> paramMap) {
		...
		if(Constant.BEI_LIAO_KAI_SHI.equals(tvVarNamePre)) {//备料开始
		...
		else if(Constant.ZHU_JI_LIU_YI_CI_TIAN_JIA_WAN_CHENG.equals(tvVarNamePre)) {
			...
			if(TriggerVar.U.equals(upRecType)) {
				...
				if(preValue==TriggerVar.DOWN) {//当上一次的变量值为0，说明这次刚上升，变量刚从0变为1，就记录一下反应釜id
					...
					Map<String, Object> zjlyctjwcUResMap = OpcUtil.readerOpcProVarByTVList(opcTVList);//根据助剂六一次添加完成变量从opc端查找对应的过程变量
		...
		else if(Constant.ZHU_JI_LIU_ER_CI_TIAN_JIA_WAN_CHENG.equals(tvVarNamePre)) {
			...
			if(TriggerVar.U.equals(upRecType)) {
				...
				if(preValue==TriggerVar.DOWN) {//当上一次的变量值为0，说明这次刚上升，变量刚从0变为1，就记录一下反应釜id
					...
					Map<String, Object> zjlectjwcUResMap = OpcUtil.readerOpcProVarByTVList(opcTVList);//根据助剂六二次添加完成变量从opc端查找对应的过程变量
			...
			if(TriggerVar.U.equals(upRecType)) {
				...
				if(preValue==TriggerVar.DOWN) {//当上一次的变量值为0，说明这次刚上升，变量刚从0变为1，就记录一下反应釜id
					...
					System.out.println("添加"+i);
				}else if(preValue==TriggerVar.UP) {//当上一次的变量值为0，说明这次刚上升，变量刚从0变为1，就记录一下反应釜id
					List<TriggerVar> opcTVList=new ArrayList<TriggerVar>();
					opcTVList.add(upYcjwjstxTV);
					Map<String, Object> ycjwjstxUResMap = OpcUtil.readerOpcProVarByTVList(opcTVList);//根据一次降温加酸提醒变量从opc端查找对应的过程变量
					List<ProcessVar> ycjwjstxUResPVList = (List<ProcessVar>)ycjwjstxUResMap.get("proVarList");
			...
			TriggerVar upEcjxlhstxTV = (TriggerVar)paramMap.get("upEcjxlhstxTV");
			String upRecType = upEcjxlhstxTV.getRecType();//获取配方类型
			if(TriggerVar.U.equals(upRecType)) {
				HashMap<String, Object> preValueFUMap = (HashMap<String,Object>)paramMap.get("preValueFUMap");
				String upVarName = upEcjxlhstxTV.getVarName();
				Float preValue = Float.valueOf(preValueFUMap.get(upVarName).toString());
				if(preValue==TriggerVar.DOWN) {//当上一次的变量值为0，说明这次刚上升，变量刚从0变为1，就记录一下反应釜id
					List<TriggerVar> opcTVList=new ArrayList<TriggerVar>();
					opcTVList.add(upEcjxlhstxTV);
	...
	private Map<String, List<TriggerVar>> getTriVarListGroupMap(List<TriggerVar> triggerVarList) {
		...
		List<TriggerVar> zjlyctjwcTVList=new ArrayList<TriggerVar>();//助剂六一次添加完成
		List<TriggerVar> zjlecblwcTVList=new ArrayList<TriggerVar>();//助剂六二次备料完成
		List<TriggerVar> zjlectjwcTVList=new ArrayList<TriggerVar>();//助剂六二次添加完成
		...
		for (TriggerVar triggerVar : triggerVarList) {//遍历全部变量对象
			...
			else if ((Constant.ZHU_JI_LIU_YI_CI_TIAN_JIA_WAN_CHENG+"_"+fyfh+"_AV").equals(varName)){//助剂六一次添加完成
			...
			else if ((Constant.ZHU_JI_LIU_ER_CI_TIAN_JIA_WAN_CHENG+"_"+fyfh+"_AV").equals(varName)){//助剂六二次添加完成
		...
		tvGroupMap.put(Constant.ZHU_JI_LIU_YI_CI_TIAN_JIA_WAN_CHENG,zjlyctjwcTVList);//助剂六一次添加完成
		tvGroupMap.put(Constant.ZHU_JI_LIU_ER_CI_BEI_LIAO_WAN_CHENG,zjlecblwcTVList);//助剂六二次备料完成
		tvGroupMap.put(Constant.ZHU_JI_LIU_ER_CI_TIAN_JIA_WAN_CHENG,zjlectjwcTVList);//助剂六二次添加完成
