...
public class OPCController {
	...
	public Map<String, Object> keepWatchOnTriggerVar() {
		...
		try {
			...
			/*
			for (TriggerVar triggerVar : triggerVarList) {
				System.out.println("==="+triggerVar.getVarName()+","+triggerVar.getVarValue());
			}
			*/
			...
			if(Constant.ALLOW_RUN_JQBLKS_TV) {
				//甲醛备料开始
			...
			if(Constant.ALLOW_RUN_JQFLWC_TV) {
				//甲醛放料完成
			...
			if(Constant.ALLOW_RUN_JJPHZZC_TV) {
				//加碱PH值正常
			...
			if(Constant.ALLOW_RUN_ZJ6YCTJWC_TV) {
				//助剂6一次添加完成
			...
			if(Constant.ALLOW_RUN_ZJ6ECBLWC_TV) {
				//助剂6二次备料完成
			...
			if(Constant.ALLOW_RUN_ZJ6ECTJWC_TV) {
				//助剂6二次添加完成
			...
			if(Constant.ALLOW_RUN_DYCBWQD_TV) {
				//第一次保温启动
			...
			if(Constant.ALLOW_RUN_DYCBWHG_TV) {
				//第一次保温合格
			...
			if(Constant.ALLOW_RUN_YCJWJSTX_TV) {
				//一次降温加酸提醒
			...
			if(Constant.ALLOW_RUN_JJPHHG_TV) {
				//加碱PH合格
			...
			if(Constant.ALLOW_RUN_ECJ215QD_TV) {
				//二次加215启动
			...
			if(Constant.ALLOW_RUN_ECJ215WC_TV) {
				//二次加215完成
				...
				List<TriggerVar> upEcj215wcTVList = getUpDownVarValueListFromList(ecj215wcTVList, TriggerVar.UP);//获取上升的二次加215完成变量
			...
			if(Constant.ALLOW_RUN_ECJSHXLTX_TV) {
				//二次加水和小料提醒
			...
			if(Constant.ALLOW_RUN_ECJSQD_TV) {
				//二次加水启动
			...
			if(Constant.ALLOW_RUN_ECJSWC_TV) {
				//二次加水完成
				...
				List<TriggerVar> upEcjswcTVList = getUpDownVarValueListFromList(ecjswcTVList, TriggerVar.UP);//获取下降的二次加水完成变量
			...
			if(Constant.ALLOW_RUN_YXYCJZJ_TV) {
				//允许一次加助剂
			...
			if(Constant.ALLOW_RUN_SYZJJLWC1_TV) {
				//所有助剂加料完成1
			...
			if(Constant.ALLOW_RUN_JFLTX_TV) {
			...
			if(Constant.ALLOW_RUN_JFLPHHG_TV) {
				//加粉料PH合格
			...
			if(Constant.ALLOW_RUN_WD85YECTLTX_TV) {
				//温度85与二次投料提醒
			...
			if(Constant.ALLOW_RUN_ECZJHCPHTX_TV) {
				//二次助剂后测PH提醒
			...
			if(Constant.ALLOW_RUN_YXECJZJ_TV) {
				//允许二次加助剂
			...
			if(Constant.ALLOW_RUN_SYZJJLWC2_TV) {
				//所有助剂加料完成2
			...
			if(Constant.ALLOW_RUN_WD98PHHG_TV) {
				//温度98PH合格
			...
			if(Constant.ALLOW_RUN_CLBSWD_TV) {
				//测量冰水雾点提醒
			...
			if(Constant.ALLOW_RUN_CSSTX_TV) {
				//测水数提醒
			...
			if(Constant.ALLOW_RUN_YXKSPJ_TV) {
				//允许开始排胶
	...
	private void addProVarByParamMap(Map<String,Object> paramMap) {
		...
		if(Constant.BEI_LIAO_KAI_SHI.equals(tvVarNamePre)) {//备料开始
		...
		else if((Constant.WEN_DU_98_PH+Constant.HE_GE).equals(tvVarNamePre)) {//温度98PH合格
			...
			if(TriggerVar.M.equals(upRecType)) {
				...
				if(preValue==TriggerVar.DOWN) {//当上一次的变量值为0，说明这次刚上升，变量刚从0变为1，就记录一下反应釜id
					...
					if (allowAdd) {
						...
						if ("ok".equals(status)) {
							...
							Float jgh = OpcUtil.getVarValueFromPVListByName(yxkspjMResPVList,ERecord.DRJG1JGH);
							...
							yxkspjMResMap = OpcUtil.readerOpcProVarByTVList(opcTVList);
							status = yxkspjMResMap.get("status").toString();
							if ("ok".equals(status)) {
								yxkspjMResPVList = (List<ProcessVar>) yxkspjMResMap.get("proVarList");
								i = processVarService.addFromList(yxkspjMResPVList);//调用添加过程接口
								System.out.println("添加" + i);
							}
						}
					}
				...
				if(preValue==TriggerVar.DOWN) {//当上一次的变量值为0，说明这次刚上升，变量刚从0变为1，就记录一下反应釜id
					...
					TriggerVar currYxkspjTV = new TriggerVar();
					currYxkspjTV.setVarName(upYxkspjTV.getVarName()+Constant.XHX+Constant.BSF_JG1);
					currYxkspjTV.setVarValue(upYxkspjTV.getVarValue());
					currYxkspjTV.setRecType(upYxkspjTV.getRecType());
					currYxkspjTV.setFId(upYxkspjTV.getFId());
					opcTVList.add(currYxkspjTV);
					...
					if (allowAdd) {
						...
						if ("ok".equals(status)) {
							...
							TriggerVar jgxzTv=new TriggerVar();
							jgxzTv.setVarName(Constant.JIAO_GUAN+Constant.XUAN_ZE+jgh+Constant.XHX+Constant.AV+Constant.XHX+Constant.BSF_JG1);
							opcTVList.add(jgxzTv);
					...
					if ("ok".equals(status)) {
						...
						yxkspjUResPVList = (List<ProcessVar>) yxkspjUResMap.get("proVarList");
						for(ProcessVar yxkspjUResPV:yxkspjUResPVList) {
							System.out.println("getVarName==="+yxkspjUResPV.getVarName()+",getVarValue="+yxkspjUResPV.getVarValue());
						}
				...
				if(preValue==TriggerVar.DOWN) {//当上一次的变量值为0，说明这次刚上升，变量刚从0变为1，就记录一下反应釜id
					...
					if (allowAdd) {
						...
						if ("ok".equals(status)) {
							...
							//获取排胶完成时间变量名
							String pjwcSjVarName = ERecord.PJWCSSYSJ;
							ProcessVar pjwcSjPV = OpcUtil.getProVarInListByVarName(pjwcSjVarName, pjwcUResPVList);
							String pjwcSjVarValue = pjwcSjPV.getUpdateTime();
							ProcessVar ptnSjPV = processVarService.getPtnValuePV(pjwcSjVarName, pjwcSjVarValue + "", pjwcSjPV);
							pjwcUResPVList.add(ptnSjPV);//将时间差对象添加到集合里
	...
	private Map<String, List<TriggerVar>> getTriVarListGroupMap(List<TriggerVar> triggerVarList) {
		...
		List<TriggerVar> ecj215wcTVList=new ArrayList<TriggerVar>();//二次加215完成
		List<TriggerVar> ecjsqdTVList=new ArrayList<TriggerVar>();//二次加水启动
		List<TriggerVar> ecjswcTVList=new ArrayList<TriggerVar>();//二次加水完成
		...
		tvGroupMap.put(Constant.ER_CI_JIA_SHUI_WAN_CHENG,ecjswcTVList);//二次加水完成
