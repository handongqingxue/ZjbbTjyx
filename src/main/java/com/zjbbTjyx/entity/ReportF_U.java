package com.zjbbTjyx.entity;

public class ReportF_U {
	
    /**
     * 自动表单设计行号
     */
    public static final int ZDBDSJ_RN=1;
    /**
     * 自动表单设计列号
     */
    public static final int ZDBDSJ_CN=1;

    /**
     * 设计号行号
     */
    public static final int SJH_RN=1;
    /**
     * 设计号列号
     */
    public static final int SJH_CN=2;

    //生产编号-3
    /**
     * 生产编号行号
     */
    public static final int SCBH_RN=3;
    /**
     * 生产编号列号
     */
    public static final int SCBH_CN=2;

    /**
     * 反应釜行号
     */
    public static final int FYF_RN=3;
    /**
     * 反应釜列号
     */
    public static final int FYF_CN=4;

    /**
     * 开始时间行号
     */
    public static final int KSSJ_RN=3;
    /**
     * 开始时间列号
     */
    public static final int KSSJ_CN=6;

    /**
     * 结束时间行号
     */
    public static final int JSSJ_RN=3;
    /**
     * 结束时间列号
     */
    public static final int JSSJ_CN=8;

    /**
     * 生产工时行号
     */
    public static final int SCGS_RN=3;
    /**
     * 生产工时列号
     */
    public static final int SCGS_CN=10;

    /**
     * 生产日期行号
     */
    public static final int SCRQ_RN=3;
    /**
     * 生产日期列号
     */
    public static final int SCRQ_CN=12;

    //1号罐用前重-5
    /**
     * 1号罐用前重行号
     */
    public static final int GYQZ1_RN=5;
    /**
     * 1号罐用前重列号
     */
    public static final int GYQZ1_CN=2;

    /**
     * 1号罐用后重行号
     */
    public static final int GYHZ1_RN=5;
    /**
     * 1号罐用后重列号
     */
    public static final int GYHZ1_CN=4;

    /**
     * 2号罐用前重行号
     */
    public static final int GYQZ2_RN=5;
    /**
     * 2号罐用前重列号
     */
    public static final int GYQZ2_CN=6;

    /**
     * 2号罐用后重行号
     */
    public static final int GYHZ2_RN=5;
    /**
     * 2号罐用后重列号
     */
    public static final int GYHZ2_CN=8;

    //YSD101-7
    /**
     * 甲醛实际进料重量行号
     */
    public static final int JQSJJLZL_RN=7;
    /**
     * 甲醛实际进料重量列号
     */
    public static final int JQSJJLZL_CN=2;

    /**
     * 甲醛备料开始时间行号
     */
    public static final int JQBLKSSJ_RN=7;
    /**
     * 甲醛备料开始时间列号
     */
    public static final int JQBLKSSJ_CN=3;

    /**
     * 甲醛放料完成时间行号
     */
    public static final int JQFLWCSJ_RN=7;
    /**
     * 甲醛放料完成时间列号
     */
    public static final int JQFLWCSJ_CN=4;

    /**
     * 甲醛放料完成反应釜温度行号
     */
    public static final int JQFLWCFYFWD_RN=7;
    /**
     * 甲醛放料完成反应釜温度列号
     */
    public static final int JQFLWCFYFWD_CN=5;

    /**
     * 加碱前PH输入值行号
     */
    public static final int JJQPHSRZ_RN=7;
    /**
     * 加碱前PH输入值列号
     */
    public static final int JJQPHSRZ_CN=6;

    /**
     * 甲醛备料开始釜称重行号
     */
    public static final int JQBLKSFCZ_RN=7;
    /**
     * 甲醛备料开始釜称重列号
     */
    public static final int JQBLKSFCZ_CN=8;

    /**
     * 甲醛放料完成釜称重行号
     */
    public static final int JQFLWCFCZ_RN=7;
    /**
     * 甲醛放料完成釜称重列号
     */
    public static final int JQFLWCFCZ_CN=9;

    /**
     * 甲醛备料开始到放料完成重量差行号
     */
    public static final int JQBLKSDFLWCZLC_RN=7;
    /**
     * 甲醛备料开始到放料完成重量差列号
     */
    public static final int JQBLKSDFLWCZLC_CN=10;

    /**
     * 甲醛备料开始到放料完成时间差行号
     */
    public static final int JQBLKSDFLWCSJC_RN=7;
    /**
     * 甲醛备料开始到放料完成时间差列号
     */
    public static final int JQBLKSDFLWCSJC_CN=11;

    //纯净水-8
    /**
     * 加水实际重量行号
     */
    public static final int JSSJZL_RN=8;
    /**
     * 加水实际重量列号
     */
    public static final int JSSJZL_CN=2;

    //YSD109-9
    /**
     * 加碱量提示行号
     */
    public static final int JJLTS_RN=9;

    /**
     * 加碱量提示列号
     */
    public static final int JJLTS_CN=2;

    /**
     * 加碱后PH输入值行号
     */
    public static final int JJHPHSRZ_RN=9;
    /**
     * 加碱后PH输入值列号
     */
    public static final int JJHPHSRZ_CN=6;

    //YSD215一次-10
    /**
     * 开始加料酸计量筒称重行号
     */
    public static final int KSJLSJLTCZ_RN=10;
    /**
     * 酸计量筒称重列号
     */
    public static final int KSJLSJLTCZ_CN=3;
    /**
     * 加碱PH值正常时间行号
     */
    public static final int JJPHZCSJ_RN=10;
    /**
     * 加碱PH值正常开始时间列号
     */
    public static final int JJPHZCSJ_CN=4;
    /**
     * 助剂六一次添加完成时间行号
     */
    public static final int ZJLYCTJWCSJ_RN=10;
    /**
     * 助剂六一次添加完成时间列号
     */
    public static final int ZJLYCTJWCSJ_CN=5;
    /**
     * 助剂六一次添加完成反应釜温度行号
     */
    public static final int ZJLYCTJWCFYFWD_RN=10;
    /**
     * 助剂六一次添加完成反应釜温度列号
     */
    public static final int ZJLYCTJWCFYFWD_CN=6;

    /**
     * 加碱PH值正常釜称重行号
     */
    public static final int JJPHZZCFCZ_RN=10;
    /**
     * 加碱PH值正常釜称重列号
     */
    public static final int JJPHZZCFCZ_CN=8;

    /**
     * 助剂六一次添加完成釜称重行号
     */
    public static final int ZJLYCTJWCFCZ_RN=10;
    /**
     * 助剂六一次添加完成釜称重列号
     */
    public static final int ZJLYCTJWCFCZ_CN=9;

    /**
     * 助剂六一次添加完成到加碱PH值正常重量差行号
     */
    public static final int ZJLYCTJWCDJJPHZZCZLC_RN=10;
    /**
     * 助剂六一次添加完成到加碱PH值正常重量差列号
     */
    public static final int ZJLYCTJWCDJJPHZZCZLC_CN=10;
    /**
     * 助剂六一次添加完成到加碱PH值正常时间差行号
     */
    public static final int ZJLYCTJWCDJJPHZZCSJC_RN=10;
    /**
     * 助剂六一次添加完成到加碱PH值正常时间差列号
     */
    public static final int ZJLYCTJWCDJJPHZZCSJC_CN=11;

    //YSD215二次-11
    /**
     * 助剂六二次备料完成酸计量筒称重行号
     */
    public static final int ZJLECBLWCSJLTCZ_RN=11;
    /**
     * 助剂六二次备料完成酸计量筒称重列号
     */
    public static final int ZJLECBLWCSJLTCZ_CN=3;

    /**
     * 助剂六二次备料完成时间行号
     */
    public static final int ZJLECBLWCSJ_RN=11;
    /**
     * 助剂六二次备料完成时间列号
     */
    public static final int ZJLECBLWCSJ_CN=4;
    /**
     * 助剂六二次添加完成时间行号
     */
    public static final int ZJLECTJWCSJ_RN=11;
    /**
     * 助剂六二次添加完成时间列号
     */
    public static final int ZJLECTJWCSJ_CN=5;
    /**
     * 助剂六二次添加完成反应釜温度行号
     */
    public static final int ZJLECTJWCFYFWD_RN=11;
    /**
     * 助剂六二次添加完成反应釜温度列号
     */
    public static final int ZJLECTJWCFYFWD_CN=6;
    /**
     * 助剂六二次备料完成釜称重行号
     */
    public static final int ZJLECBLWCFCZ_RN=11;
    /**
     * 助剂六二次备料完成釜称重列号
     */
    public static final int ZJLECBLWCFCZ_CN=8;
    /**
     * 助剂六二次添加完成釜称重行号
     */
    public static final int ZJLECTJWCFCZ_RN=11;
    /**
     * 助剂六二次添加完成釜称重列号
     */
    public static final int ZJLECTJWCFCZ_CN=9;
    /**
     * 助剂六二次添加完成到备料完成重量差行号
     */
    public static final int ZJLECTJWCDBLWCZLC_RN=11;
    /**
     * 助剂六二次添加完成到备料完成重量差列号
     */
    public static final int ZJLECTJWCDBLWCZLC_CN=10;
    /**
     * 助剂六二次添加完成到备料完成时间差行号
     */
    public static final int ZJLECTJWCDBLWCSJC_RN=11;
    /**
     * 助剂六二次添加完成到备料完成时间差列号
     */
    public static final int ZJLECTJWCDBLWCSJC_CN=11;
    
    //YSD103-13
    /**
     * 粉料重量设定行号
     */
    public static final int FLZLSD_RN=13;
    /**
     * 粉料重量设定列号
     */
    public static final int FLZLSD_CN=3;
    /**
     * 加粉料提醒釜尿素放料阀上升沿时间行号
     */
    public static final int JFLTXFNSFLFSSYSJ_RN=13;
    /**
     * 加粉料提醒釜尿素放料阀上升沿时间列号
     */
    public static final int JFLTXFNSFLFSSYSJ_CN=4;
    /**
     * 加粉料提醒釜尿素放料阀下降沿时间行号
     */
    public static final int JFLTXFNSFLFXJYSJ_RN=13;
    /**
     * 加粉料提醒釜尿素放料阀下降沿时间列号
     */
    public static final int JFLTXFNSFLFXJYSJ_CN=5;
    /**
     * 加粉料提醒釜尿素放料阀下降沿反应釜温度行号
     */
    public static final int JFLTXFNSFLFXJYFYFWD_RN=13;
    /**
     * 加粉料提醒釜尿素放料阀下降沿反应釜温度列号
     */
    public static final int JFLTXFNSFLFXJYFYFWD_CN=6;
    /**
     * 加粉料PH输入值行号
     */
    public static final int JFLPHSRZ_RN=13;
    /**
     * 加粉料PH输入值列号
     */
    public static final int JFLPHSRZ_CN=8;
    /**
     * 加粉料提醒釜尿素放料阀上升沿釜称重行号
     */
    public static final int JFLTXFNSFLFSSYFCZ_RN=13;
    /**
     * 加粉料提醒釜尿素放料阀上升沿釜称重列号
     */
    public static final int JFLTXFNSFLFSSYFCZ_CN=9;

    /**
     * 加粉料提醒釜尿素放料阀下降沿釜称重行号
     */
    public static final int JFLTXFNSFLFXJYFCZ_RN=13;
    /**
     * 加粉料提醒釜尿素放料阀下降沿釜称重列号
     */
    public static final int JFLTXFNSFLFXJYFCZ_CN=10;

    /**
     * 釜尿素放料阀上升沿到下降沿重量差行号
     */
    public static final int FNSFLFSSYDXJYZLC_RN=13;
    /**
     * 釜尿素放料阀上升沿到下降沿重量差列号
     */
    public static final int FNSFLFSSYDXJYZLC_CN=11;

    /**
     * 釜尿素放料阀上升沿到下降沿时间差行号
     */
    public static final int FNSFLFSSYDXJYSJC_RN=13;
    /**
     * 釜尿素放料阀上升沿到下降沿时间差列号
     */
    public static final int FNSFLFSSYDXJYSJC_CN=12;



    //开始升温-15
    /**
     * 升温开始时间行号
     */
    public static final int SWKSSJ_RN=15;
    /**
     * 升温开始时间列号
     */
    public static final int SWKSSJ_CN=4;
    /**
     * 蒸汽压力行号
     */
    public static final int ZQYL_RN=15;
    /**
     * 蒸汽压力列号
     */
    public static final int ZQYL_CN=6;
    /**
     * 升温完成到升温开始时间差行号
     */
    public static final int SWWCDKSSJC_RN=15;
    /**
     * 升温完成到升温开始时间差列号
     */
    public static final int SWWCDKSSJC_CN=8;
    //升温至高温度-16
    /**
     * 升温完成时间行号
     */
    public static final int SWWCSJ_RN=16;
    /**
     * 升温完成时间列号
     */
    public static final int SWWCSJ_CN=4;
    /**
     * 升温完成反应釜温度行号
     */
    public static final int SWWCFYFWD_RN=16;
    /**
     * 升温完成反应釜温度列号
     */
    public static final int SWWCFYFWD_CN=6;

    /**
     * 温度98PH行号
     */
    public static final int WD98PH_RN=16;
    /**
     * 温度98PH行号
     */
    public static final int WD98PH_CN=8;
    
    //一次保温10分钟测PH-17
    /**
     * 第一次保温启动时间行号
     */
    public static final int DYCBWQDSJ_RN=17;
    /**
     * 第一次保温启动时间列号
     */
    public static final int DYCBWQDSJ_CN=4;
    /**
     * 第一次保温启动反应釜温度行号
     */
    public static final int DYCBWQDFYFWD_RN=17;
    /**
     * 第一次保温启动反应釜温度列号
     */
    public static final int DYCBWQDFYFWD_CN=6;
    /**
     * 第一次保温完成PH录入  ？？？？
     */

    //降温开始-18
    /**
     * 第一次保温合格时间行号
     */
    public static final int DYCBWHGSJ_RN=18;
    /**
     * 第一次保温合格时间列号
     */
    public static final int DYCBWHGSJ_CN=4;
    /**
     * 第一次保温合格反应釜温度行号
     */
    public static final int DYCBWHGFYFWD_RN=18;
    /**
     * 第一次保温合格反应釜温度列号
     */
    public static final int DYCBWHGFYFWD_CN=6;
    
    //降温停止-19
    /**
     * 一次降温加酸提醒时间行号
     */
    public static final int YCJWJSTXSJ_RN=19;
    /**
     * 一次降温加酸提醒时间列号
     */
    public static final int YCJWJSTXSJ_CN=4;
    /**
     * 一次降温加酸提醒反应釜温度行号
     */
    public static final int YCJWJSTXFYFWD_RN=19;
    /**
     * 一次降温加酸提醒反应釜温度列号
     */
    public static final int YCJWJSTXFYFWD_CN=6;
    /**
     * 一次降温加酸提醒到第一次保温合格时间差行号
     */
    public static final int YCJWJSTXDDYCBWHGSJC_RN=19;
    /**
     * 一次降温加酸提醒到第一次保温合格时间差列号
     */
    public static final int YCJWJSTXDDYCBWHGSJC_CN=8;
    /**
     * 降温时间  ？？？？
     */

    //加酸并计时-20
    /**
     * 一次降温加酸量行号
     */
    public static final int YCJWJSL_RN=20;
    /**
     * 一次降温加酸量列号
     */
    public static final int YCJWJSL_CN=4;
    /**
     * 一次降温加酸合格行号
     */
    public static final int YCJWJSHGFYFWD_RN=20;
    /**
     * 一次降温加酸合格列号
     */
    public static final int YCJWJSHGFYFWD_CN=6;
    /**
     * 一次降温加酸PH输入行号
     */
    public static final int YCJWJSHGSSYYCJWJSPHSZ_RN=20;
    /**
     * 一次降温加酸PH输入列号
     */
    public static final int YCJWJSHGSSYYCJWJSPHSZ_CN=8;
    
    //冰水雾点-21
    /**
     * 测量冰水雾点输入值行号
     */
    public static final int CLBSWDSRZ_RN=21;
    /**
     * 测量冰水雾点输入值列号
     */
    public static final int CLBSWDSRZ_CN=6;

    /**
     * 保温分钟计时1行号
     */
    public static final int BWFZJS1_RN=21;
    /**
     * 保温分钟计时1列号
     */
    public static final int BWFZJS1_CN=9;
    //20度雾点-22
    /**
     * 测20雾点输入值行号
     */
    public static final int C20WDSRZ_RN=22;
    /**
     * 测20雾点输入值列号
     */
    public static final int C20WDSRZ_CN=5;
    /**
     * 保温分钟计时2行号
     */
    public static final int BWFZJS2_RN=22;
    /**
     * 保温分钟计时1列号
     */
    public static final int BWFZJS2_CN=9;
    //降温-23
    /**
     * 一次降温加酸提醒下降沿一次降温加酸PH输入行号
     */
    public static final int YCJWJSTXXJYYCJWJSPHSR_RN=23;
    /**
     * 一次降温加酸PH输入列号
     */
    public static final int YCJWJSTXXJYYCJWJSPHSR_CN=6;
    
    //加碱-24
    /**
     * 加碱量范围下限行号
     */
    public static final int JJLFWXX_RN=24;
    /**
     * 加碱量范围下限列号
     */
    public static final int JJLFWXX_CN=4;
    /**
     * 加碱PH输入行号
     */
    public static final int JJPHSR_RN=24;
    /**
     * 加碱PH输入列号
     */
    public static final int JJPHSR_CN=7;
    
    //70度终止降温-25
    /**
     * 二次投粉时间行号
     */
    public static final int ECTFSJ_RN=25;
    /**
     * 二次投粉时间列号
     */
    public static final int ECTFSJ_CN=4;
    /**
     * 二次投粉到第一次保温合格时间行号
     */
    public static final int ECTFDDYCBWHGSJC_RN=25;
    /**
     * 二次投粉到第一次保温合格时间列号
     */
    public static final int ECTFDDYCBWHGSJC_CN=9;
    //二次粉料加入-26
    /**
     * 二次加料粉料1重量设定行号
     */
    public static final int ECJFL1ZLSD_RN=26;
    /**
     * 二次加料粉料1重量设定列号
     */
    public static final int ECJFL1ZLSD_CN=2;
    /**
     * 二次投粉釜尿素放料阀上升沿时间行号
     */
    public static final int ECTFFNSFLFSSYSJ_RN=26;
    /**
     * 二次投粉釜尿素放料阀上升沿时间列号
     */
    public static final int ECTFFNSFLFSSYSJ_CN=4;
    /**
     * 二次投粉釜尿素放料阀下降沿时间行号
     */
    public static final int ECTFFNSFLFXJYSJ_RN=26;
    /**
     * 二次投粉釜尿素放料阀下降沿时间列号
     */
    public static final int ECTFFNSFLFXJYSJ_CN=5;
    /**
     * 二次投粉釜尿素放料阀下降沿反应釜温度行号
     */
    public static final int ECTFFNSFLFXJYFYFWD_RN=26;
    /**
     * 二次投粉釜尿素放料阀下降沿反应釜温度列号
     */
    public static final int ECTFFNSFLFXJYFYFWD_CN=6;
    
    //助剂6加入-28
    /**
     * 保温后加助剂6量设定行号
     */
    public static final int BWHJZJ6LSD_RN=28;
    /**
     * 保温后加助剂6量设定列号
     */
    public static final int BWHJZJ6LSD_CN=2;
    /**
     * 二次加215启动时间行号
     */
    public static final int ECJ215QDSJ_RN=28;
    /**
     * 二次加215启动时间列号
     */
    public static final int ECJ215QDSJ_CN=4;
    /**
     * 二次加215完成行号
     */
    public static final int ECJ215WC_RN=28;
    /**
     * 二次加215完成列号
     */
    public static final int ECJ215WC_CN=5;
    /**
     * 二次加小料和水提醒下降沿反应釜温度行号
     */
    public static final int ECJXLHSTXXJYFYFWD_RN=28;
    /**
     * 二次加小料和水提醒下降沿反应釜温度列号
     */
    public static final int ECJXLHSTXXJYFYFWD_CN=6;
    
    //水加入-29
    /**
     * 保温后加水量设定行号
     */
    public static final int BWHJSLSD_RN=29;
    /**
     * 保温后加水量设定列号
     */
    public static final int BWHJSLSD_CN=2;
    /**
     * 二次加水启动时间行号
     */
    public static final int ECJSQDSJ_RN=29;
    /**
     * 二次加水启动时间列号
     */
    public static final int ECJSQDSJ_CN=4;
    /**
     * 二次加水完成时间行号
     */
    public static final int ECJSWCSJ_RN=29;
    /**
     * 二次加水完成时间列号
     */
    public static final int ECJSWCSJ_CN=5;
    
    //冷却至结束-30
    /**
     * 反应结束时间行号
     */
    public static final int FYJSSJ_RN=30;
    /**
     * 反应结束时间列号
     */
    public static final int FYJSSJ_CN=4;
    /**
     * 反应结束反应釜温度行号
     */
    public static final int FYJSFYFWD_RN=30;
    /**
     * 反应结束反应釜温度列号
     */
    public static final int FYJSFYFWD_CN=5;
    
    //生产总重-32
    /**
     * 允许开始排胶釜称重行号
     */
    public static final int YXKSPJFCZ_RN=32;
    /**
     * 允许开始排胶釜称重列号
     */
    public static final int YXKSPJFCZ_CN=2;
    /**
     * 终检水数行号
     */
    public static final int ZJSH_RN=33;
    /**
     * 终检水数列号
     */
    public static final int ZJSH_CN=4;
    /**
     * 终检PH行号
     */
    public static final int ZJPH_RN=33;
    /**
     * 终检PH列号
     */
    public static final int ZJPH_CN=5;
    
    private Integer Id;
    private String RowNumber;//行号
    private String ColNumber;//列号
    private String Value;//值
    private String BatchID;//生产编号
    private String CreateTime;//创建时间
    private String Remark;//备注

    public Integer getId() {
        return Id;
    }

    public void setId(Integer id) {
        Id = id;
    }

    public String getRowNumber() {
        return RowNumber;
    }

    public void setRowNumber(String rowNumber) {
        RowNumber = rowNumber;
    }

    public String getColNumber() {
        return ColNumber;
    }

    public void setColNumber(String colNumber) {
        ColNumber = colNumber;
    }

    public String getValue() {
        return Value;
    }

    public void setValue(String value) {
        Value = value;
    }

    public String getBatchID() {
        return BatchID;
    }

    public void setBatchID(String batchID) {
        BatchID = batchID;
    }

    public String getCreateTime() {
        return CreateTime;
    }

    public void setCreateTime(String createTime) {
        CreateTime = createTime;
    }

    public String getRemark() {
        return Remark;
    }

    public void setRemark(String remark) {
        Remark = remark;
    }
}
