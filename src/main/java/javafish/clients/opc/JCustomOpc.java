...
abstract public class JCustomOpc {
  ...
  /** opc server name     opc服务器名称  */
  protected String serverProgID;
  
  /** user name of opc server    opc服务器的用户名*/
  protected String serverClientHandle;
  
  /** use log4j messaging  	使用log4j消息传递*/
  protected boolean useStandardReporting = true;
  
  /** counter of messages  计数器的消息*/
  protected int logPkg = 0;
  
  /** common logger   记录器*/
  protected Log log = LogFactory.getLog(getClass());
  
  /** properties file     文件属性*/
  protected static Properties props;

  /** report event listeners   报告事件监听器*/
  protected EventListenerList reportListeners = new EventListenerList();
  
  /** important: specify OPC object in dll-library (not modify)    重点:在dll库中指定OPC对象(不修改) */
  private int id;

  static {
    // load class properties   加载类属性
